...
def find_subtel(pval_arr):
	on=0
	penalty=0
	for i in range(2,len(pval_arr)-2):
		tot=float(pval_arr[i-2])+float(pval_arr[i-1])+float(pval_arr[i])+float(pval_arr[i+1])+float(pval_arr[i+2])
		mov_avg=tot/5
		#print(mov_avg)
		if(mov_avg>0.5):
			penalty+=1
